...
function displayBudgets(listarray: string[], i: number = 0, allBudgets: string = ""): any {
    ...
    if (i != listarray.length) {
        ...
        const addHtml =
            `<div class= "budgets-container" id="${listarray[i]}">
        <h1>${name}</h1>
...
function addExp(budgetname: string) {
    ...
    const expense: number = Number((<HTMLInputElement>document.getElementById(`${budgetname}addexp`)).value) + Number(budgetinfo.spent);
