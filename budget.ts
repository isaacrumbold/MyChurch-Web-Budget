const currentUser: string = sessionStorage.getItem("useris")!;
const userInfo = JSON.parse(localStorage.getItem(currentUser)!);

const userBudgets = JSON.parse(
  localStorage.getItem(userInfo.username + "budgets")!
);
...
function displayBudgets(
  ...
  if (i != listarray.length) {
    ...
    const addHtml = `<div class= "budgets-container" id="${listarray[i]}">
        ...
        <button type="button" id="addexpense" onclick = "addExp('${
          listarray[i]
        }')">Add Expense</button>
        <input id="${
          listarray[i]
        }addexp" type="number" placeholder="Add Expense" name="addexpbtn"/>
        <button type="button" id="deletebudget" onclick="deletebudget('${
          listarray[i]
        }')">Delete</button>
      </div>`;
    ...
    let htmldoc = <HTMLElement>document.getElementById("budgets");
...
function addExp(budgetname: string) {
  const budgetinfo = JSON.parse(localStorage.getItem(budgetname)!);
  const expense: number =
    Number(
      (<HTMLInputElement>document.getElementById(`${budgetname}addexp`)).value
    ) + Number(budgetinfo.spent);
...
function deletebudget(budgetname: string) {
  const budgetarr: string[] = userBudgets;
  const index = budgetarr.indexOf(budgetname);
  budgetarr.splice(index, 1);
  localStorage.removeItem(budgetname);
  localStorage.setItem(
    userInfo.username + "budgets",
    JSON.stringify(budgetarr)
  );
